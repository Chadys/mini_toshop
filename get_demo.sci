function make_demo(img, file_path)
	...
	carre = img(1:min(size(img,1),size(img,2)),1:min(size(img,1),size(img,2)),:)
	imwrite(rotation(carre,150), file_path+"/20.png")
	...
	imwrite(fisheye(carre), file_path+"/42.png")
	imwrite(time_warp(carre), file_path+"/43.png")
